...
:------------ | :-----------------------------------------------------------
`canvas_network`  | The region of IP space visualized by the plot. By default, the entire IPv4 space is visualized. This argument allows you to zoom into regions of IP space or visualize IPv6 space.
`pixel_prefix`  | The network prefix length corresponding to the grid's pixel. Increasing this argument effectively increases the resolution of the plot.
...
It contains an `ip` column with the original `ip_address()` vector, and `x` and `y` columns with the Cartesian coordinates.
...
```{r, out.width='80%', fig.asp=0.8, fig.width=7, eval=rlang::is_installed("ggfittext")}
iana_ipv4 %>%
  ggplot(aes(xmin = network$xmin, ymin = network$ymin, xmax = network$xmax, ymax = network$ymax)) +
  geom_rect(aes(fill = allocation)) +
  ggfittext::geom_fit_text(aes(label = label), reflow = TRUE) +
...
iana_ipv4 %>%
  ggplot() +
  geom_rect(aes(
    xmin = network$xmin, ymin = network$ymin, xmax = network$xmax, ymax = network$ymax,
    fill = allocation
  )) +
  geom_label(aes(
    x = (network$xmin + network$xmax) / 2,
    y = (network$ymin + network$ymax) / 2,
    label = label
  ), data = ~ filter(.x, prefix_length(network) <= 6)) +
  scale_fill_brewer(palette = "Accent", name = NULL) +
  coord_ip() +
  theme_ip_dark()
```
...
They can then extract the relevant information from the data frame column.
This is easier because it's also the name of the original `ip_address()` or `ip_network()` column in the input data set.
