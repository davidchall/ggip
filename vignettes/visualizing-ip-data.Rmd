...
```{r, echo=FALSE, out.width="100%", fig.cap="Raw bits of `192.168.0.124`."}
...
This would effectively filter addresses where the leading 8 bits match the specified network (shown in yellow below), thereby reducing the number of discrete levels to $2^{24}$ (roughly 16.8 million).
...
This would effectively neglect the trailing 8 bits of the 32-bit address (shown in blue below), thereby further reducing the number of discrete levels to $2^{16}$ (65,536).

```{r, echo=FALSE, out.width="100%", fig.cap='Reduced bits of `192.168.0.124` using a `canvas_network` of `192.0.0.0/8` and a `pixel_prefix` of 24.'}
